...
export class ManageBeneficiariesPage implements OnInit, OnDestroy {
  ...
  constructor( public beneficiaryService: BeneficiariesService, private alertController: AlertController,
     private modalCtrl: ModalController, private route: ActivatedRoute,   private router: Router, private platform: Platform) { }

     async openModal(accountName, accountNumber, benbankName,beneficiaryId, benBankCode ) {
      ...
      this.selectedBeneficiary = {   //pass as prop instead of sessionstorage
...
sessionStorage.setItem('selectedBeneficiary', JSON.stringify(this.selectedBeneficiary));
sessionStorage.setItem('beneficiaryId', JSON.stringify(this.selectedBeneficiary.benID));
  ...
  ngOnInit() {
    this.httpSubscriptions.push(this.beneficiaryService.getBeneficiaryList()
  .subscribe(
    data=>{
